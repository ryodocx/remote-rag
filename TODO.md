...
- [x] **他のツール提供の検討**: 現在の `hybrid_search` ツールに加え、文書の要約ツールやメタデータ抽出ツールなどの提供を検討する。 (参照: `README.md` の MCPツール説明部分)
  - `list_wiki_pages` を追加済み
- [ ] **モニタリングと可観測性の強化**: OpenTelemetry または Prometheus を用いたメトリクス出力（レイテンシ、キャッシュヒット率など）の導入。
