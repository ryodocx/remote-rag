<div align="center">
  ...
  <p>
    <b>🛡️ 堅牢なセキュリティ (OAuth 2.0 & JWKS)</b> &nbsp;•&nbsp; 
...
*   **🛡️ セキュアな認証プロキシ (Zero Trust Ready)**
    Caddy + 独自実装の Auth Helper により、MCP通信の直前で OAuth 2.0 のトークン検証を実行。**Token Introspection (RFC 7662)** に加え、**JWKSによる超高速なローカルJWT検証**にも対応しており、ベンダーに依存しない多層防御を実現します。
*   **👤 属性ベースのアクセス制御 (ABAC)**
...
| **[GitHub設定例](docs/GITHUB_SETUP.md)** | GitHub OAuth を利用する場合のアーキテクチャ上の制限と、Auth0 等のブローカーを介した設定手順。 |
